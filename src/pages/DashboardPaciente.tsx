...
import { Calendar, Clock, User, Pill, ArrowLeft } from 'lucide-react';
...
export default function DashboardPaciente() {
  ...
  // 1. CARREGA CONSULTAS
  apiService.getConsultasDoPaciente(user.idPaciente)
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-700 font-medium">Carregando consultas e receitas...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="bg-white rounded-3xl shadow-2xl p-8 mb-8">
          <div className="flex items-center justify-between mb-6">
            <button
              onClick={() => { apiService.logout(); navigate('/acesso-paciente'); }}
            ...
            <button
              ...
              className="bg-gradient-to-r from-green-600 to-emerald-600 text-white px-6 py-3 rounded-xl font-bold hover:from-green-700 hover:to-emerald-700 transition shadow-lg"
            >
              + Agendar Consulta
            </button>
          </div>
        ...
        <div className="space-y-6">
          {consultas.length === 0 ? (
            <div className="bg-white rounded-3xl shadow-xl p-12 text-center">
              ...
              <p className="text-xl text-gray-600">Nenhuma consulta encontrada.</p>
            </div>
          ) : (
            consultas.map((consulta) => {
              const receitaDaConsulta = receitas.find(r => r.consulta?.idConsulta === consulta.idConsulta);

              return (
                <div key={consulta.idConsulta} className="bg-white rounded-3xl shadow-xl p-6">
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <h3 className="text-xl font-bold text-gray-800">{consulta.areaMedica}</h3>
                    ...
                    <span className={`px-4 py-2 rounded-full text-sm font-bold ${getStatusColor(consulta.status)}`}>
                      {consulta.status ?? '—'}
                    </span>
                  </div>
                  ...
                  <div className="flex gap-3">
                    {(consulta.status === 'AGENDADA' || consulta.status === 'REALIZADA') && (
                      <button
                        onClick={() => navigate(`/editar-consulta?id=${consulta.idConsulta}`)}
                    ...
                    {receitaDaConsulta && (
                      <button
                        onClick={() => navigate(`/receita?id=${consulta.idConsulta}`)}
